...
## 开发计划
- [ ] 代码上一版本保留：修改链接覆盖代码缓存前保留上一版，可通过导出取回，避免停服网站项目的唯一副本被覆盖后永久丢失
- [ ] 数据自检覆盖失效项目：链接指向已停服网站、且本地无代码缓存的项目已无法执行，需在自检中单独列出
- [ ] 拆分 `CommandPastebin.kt`：按指令族拆分，降低单文件体积
- [ ] 渲染进程按占用量依次终止：整机内存超出安全限制时，优先终止占用最高的渲染进程，从高到低依次终止直至回到限制以内
- [ ] 个人收藏：每人维护一份常用项目列表，便于查看与快速执行
- [ ] 定时执行：由管理员为项目设置每日固定时刻，无人触发地执行并将输出投递到指定群；执行时以系统身份标识来源，需同步更新项目接口文档
